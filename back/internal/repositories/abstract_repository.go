...
func BuildNewRepository[T any, Q any](ctx context.Context, collection string, connection *mongo.Database, logger *zerolog.Logger) ports.Repository[T, Q] {
...
func (r *AbstractRepository[T, Q]) FindOne(ctx context.Context, opts ports.FindOneOpts, result *T) error {
	filterBson := bson.D{}
...
func (r *AbstractRepository[T, Q]) FindMany(ctx context.Context, opts ports.FindManyOpts, result *[]T, returnCount bool) (*int64, error) {
	filterBson := bson.D{}
	for key, value := range opts.Filter {
		filterBson = append(filterBson, bson.E{Key: key, Value: validateOID(value)})
	}
...
func (r *AbstractRepository[T, Q]) DeleteOne(ctx context.Context, opts ports.DeleteOpts) (bool, error) {
